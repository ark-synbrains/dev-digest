...
5. Put SMTP + Anthropic + `NEWSLETTER_TO_EMAILS` secrets in the Cloud Agent
   environment used by the automation
6. Save and enable

Alternatively, in a local Cursor chat on this repo, run `/automate` and say:

> Create a scheduled automation every 12 hours that runs `cd agent && npm ci && npm start` in ark-synbrains/dev-digest and emails the /dev/digest newsletter via SMTP to NEWSLETTER_TO_EMAILS. Do not open a PR.
