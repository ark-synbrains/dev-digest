...
5. Put `RESEND_API_KEY` and `NEWSLETTER_TO_EMAILS` in the Cloud Agent
   environment secrets used by the automation
6. Save and enable

Alternatively, in a local Cursor chat on this repo, run `/automate` and say:

> Create a scheduled automation every 12 hours that runs `cd agent && npm ci && npm start` in ark-synbrains/dev-digest. It should pull newsletter items from Hacker News, Dev.to, and curated RSS, then email via Resend to NEWSLETTER_TO_EMAILS. Do not open a PR.
